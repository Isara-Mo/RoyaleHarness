...
当前版本：**`v0.1.0-preview`，研究预览版**。已有实机功能验证和发布副本离线验证，
尚未完成独立电脑／全新模拟器的完整安装与实战验收。
...
这些能力的验证范围各不相同。具体边界见 [架构说明](docs/ARCHITECTURE.md)，
已有证据见 [验证记录](docs/VALIDATION.md)。
...
- General 使用完整目录，但不代表全部卡牌、英雄技能与塔兵机制都经过实机验证。
...
| Python | 3.12 |
| 推理环境 | 本机验证组合为 PyTorch 2.11.0 + CUDA 12.8；提供 CPU 安装选项，实时性能未验收 |
...
2. 安装 Python 3.12，在本项目目录打开 PowerShell：
   ...
   CPU 使用 `-TorchBuild cpu`，并将配置中的 `device` 改为 `cpu`；CPU 延迟未经实战评估。
...
英雄火枪手未校准技能按钮时，技能点击会禁用，不能将其视作完整英雄实战验收。
...
**现阶段已经完整发挥了 FirstLight 模型实力吗？**

尚不能这样宣称。默认输入方案用于尽量对齐已核对的原版规则，
仍有采样、完整可见性和特殊机制覆盖差异；“慢半拍”和重复施法也未被证明彻底解决。
离线推理通过说明接口和运行检查通过，不等于实战强度提升。
...
这些检查不替代全新模拟器上的首次安装与整局实战；详见 [验证记录](docs/VALIDATION.md)
和 [机器可读记录](release-validation.json)。
...
源码中的 `Experimental` 构建开关仅供开发研究，不能视为已验收功能。
...
- [ ] 完成独立电脑／全新模拟器的首次安装和完整实战验收。
- [ ] 对当前支持卡组进行持续实战评估，区分执行问题和策略选择。
- [ ] 在有验证样本的前提下扩大英雄、觉醒和塔兵机制覆盖。
- [ ] 引入视觉感知、对象跟踪与状态估计，评估与现有模型输入的兼容性。

后续功能的顺序取决于验证结果，目前没有承诺视觉版或全卡支持的发布时间。
...
行为变更需要相应回归验证。未经实机验证的功能应明确标记，避免将目录支持等同于机制验收。
